...
export default function Health(){
    const [condition,setSelected] = useState([])
    const [condtions] = useState([
        "Scoliosis",
        "Lower Back Pain",
        "Ligament Tear",
        "Build Strength",
    ...
    <RegisterLayout title={"Interests"} goBack={()=>console.log("pressed")}>
        <View style={style.form}>
           <View>
            <Text style={style.header}>Do you have any Serious Health issue?</Text>
            {condtions.map((con,i)=>(
                 <TouchableWithoutFeedback key={i}>
                 <View style={{...style.input,marginBottom:13}}>
                     <Text style={{fontFamily:"Poppins_600SemiBold",color:colors.white,fontSize:18}}>{con}</Text>
                 </View>
